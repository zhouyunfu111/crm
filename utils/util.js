...
export default {
	...
	uploadImgs(filePath, opt, successCallback, errorCallback) {
		...
		let count = opt.count || 1,
			...
			pid = opt.pid,
			model = opt.model;
		let urlPath = baseUrl + '/api/front/upload/image' + "?model=" + model +
			"&pid=" + pid
		uni.uploadFile({
			url: urlPath,
			filePath: filePath,
			...
			header: {
				...
				// #endif
				[tokenName]: store.state.user.token
			},
			success: function(res) {
				uni.hideLoading();
	...
	uploadImageOne: function(opt, successCallback, errorCallback) {
		...
		let count = opt.count || 1,
			...
			is_load = opt.is_load || true,
			uploadUrl = opt.url || '',
			inputName = opt.name || 'multipart',
			pid = opt.pid || 0,
			model = opt.model || 'shop';
		...
		uni.chooseImage({
			...
			success: function(res) {
				...
				let urlPath = baseUrl + '/crm/upload/image' + "?model=" + model +
					"&pid=" + pid
	...
	// 根据字典数据类型和value值返回对应的label名称
	getDictLabelByValue(dictType,id){
		try {
			const dictData = store.state.user.dictDataType[dictType]
			if(typeof dictData === 'undefined' || dictType === '' || (id === '' && typeof id === 'undefined')) {
				throw new Error('没有找到对应的数据或者参数有问题')
			}
			return dictData.find(item => String(item.dictValue) === String(id))?.dictLabel || ''
		} catch(e) {
